...
export default function Dashboard() {
  ...
  const onDelete = async () => {
    let isSucceed = true;
    try {
      console.log("waiting for delete receipt");
      await deleteReceipt(deleteReceiptId);
      console.log("delete receipt is called with id: " + deleteReceiptId);
      await deleteImage(deleteReceiptImageBucket);
      console.log("delete image is callled");
    } catch (error) {
      isSucceed = false;
    }
  ...
  return (
    ...
    <div>
      ...
      <Dialog open={action === RECEIPTS_ENUM.delete} onClose={resetDelete}>
        <Typography variant="h4" className={styles.title}>DELETE ACTIVITY</Typography>
        <DialogContent>
            <Alert severity="error">This will permanently delete your activity from the log!</Alert>
        </DialogContent>
      ...
      {receipts.map((receipt) => (
              <div key={receipt.id}>
                ...
                <ReceiptRow
                  ...
                  onDelete={() =>
                    onClickDelete(receipt.id, receipt.imageBucket)
                  }
                />
              </div>
        ))}
