...
export default function ExpenseDialog(props) {
  ...
  const isDisabled = () => formFields.fileName === DEFAULT_FILE_NAME || !formFields.date || formFields.locationName.length === 0 
                     || formFields.work.length === 0 || formFields.equipment.length === 0 || formFields.issues.length === 0;
  ...
  const handleSubmit = async() => {
    ...
    try{
      if(isEdit) {
        ...
        await updateReceipt(formFields.id, authUser.uid, formFields.date, formFields.locationName, formFields.work,
          formFields.equipment, formFields.issues, formFields.imageBucket)
      } else {
        const bucket = await uploadImage(formFields.file, authUser.uid);
        await addReceipt(authUser.uid, formFields.date, formFields.locationName, formFields.work,
          formFields.equipment, formFields.issues, bucket);
      }
    ...
    <Dialog classes={{paper: styles.dialog}}
      ...
      <DialogContent className={styles.fields}>
        <Stack direction="row" spacing={2} className={styles.receiptImage}>
          {(isEdit && !formFields.fileName) && <Avatar alt="activity image" src={formFields.imageUrl} sx={{ marginRight: '1em' }}/> }
        ...
        <TextField color="tertiary" label="Type of work done" variant="standard" value={formFields.address} onChange={(event) => updateFormField(event, 'work')} />
        <TextField color="tertiary" label="Equipment" variant="standard" value={formFields.items} onChange={(event) => updateFormField(event, 'equipment')} />
        <TextField color="tertiary" label="Issues" variant="standard" value={formFields.issues} onChange={(event) => updateFormField(event, 'issues')} />
      </DialogContent>
      <DialogActions>
        {isSubmitting ? 
